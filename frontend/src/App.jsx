...
function PreparationDetail({ preparationId, onBack }) {
  ...
  const [fieldLabel, setFieldLabel] = useState(''); const [autofillKey, setAutofillKey] = useState(''); const [consent, setConsent] = useState(false)
  const { data, isLoading, error } = useQuery({ queryKey: ['preparation', preparationId], queryFn: () => api(`/api/preparations/${preparationId}`) })
  const add = useMutation({ mutationFn: () => api(`/api/preparations/${preparationId}/requirements`, { method: 'POST', body: JSON.stringify({ title, is_required: true }) }), onSuccess: () => { setTitle(''); queryClient.invalidateQueries({ queryKey: ['preparation', preparationId] }); queryClient.invalidateQueries({ queryKey: ['preparations'] }) } })
  const confirmSource = useMutation({ mutationFn: checked => api(`/api/preparations/${preparationId}`, { method: 'PUT', body: JSON.stringify({ source_confirmed: checked }) }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['preparation', preparationId] }) })
  const extract = useMutation({ mutationFn: () => Promise.all([api(`/api/preparations/${preparationId}/extract`, { method: 'POST' }), api(`/api/preparations/${preparationId}/form-fields/extract`, { method: 'POST' })]), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['preparation', preparationId] }); queryClient.invalidateQueries({ queryKey: ['preparations'] }) } })
  ...
  const addField = useMutation({ mutationFn: () => api(`/api/preparations/${preparationId}/form-fields`, { method: 'POST', body: JSON.stringify({ label: fieldLabel, autofill_profile_key: autofillKey || null, autofill_consent: consent }) }), onSuccess: () => { setFieldLabel(''); setAutofillKey(''); setConsent(false); queryClient.invalidateQueries({ queryKey: ['preparation', preparationId] }) } })
...
export default function App() {
  ...
  return <div className="shell"><header><button className="brand" onClick={() => setView('home')}><span>M</span>목포 청년 정책</button><nav><button onClick={() => setView('search')}>정책 검색</button><button onClick={() => setView('chat')}>AI 상담</button>{user && <><button onClick={() => setView('policy')}>맞춤 정책</button><button onClick={() => setView('wishlist')}>관심 정책</button><button onClick={() => setView('preparations')}>신청 준비함</button><button onClick={() => setView('notification')}>새 알림</button><button onClick={logout}>로그아웃</button></>}</nav></header><main>{content}</main><footer>목포 거주 청년을 위한 맞춤 정책 알림 서비스</footer></div>
}
